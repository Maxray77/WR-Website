...
- **TWO 2026 register files exist — check mtimes, the authoritative one FLIPS.** As of **2026-08-02** `Data/Intake Records/2026/Cases 2026.xlsx` is **authoritative** (ends **Jul 31**, #37,650→#40,570, **2,921 cases**); `Data/Current Year/Cases 2026.xlsx` is now the stale one (ends Jun 18, 2,392 cases). This is the **reverse** of the 2026-06-18 state, when Current Year led. Don't trust either path by name — compare last-modified dates + max case number before using one. (**#40,000 = Shikra, 2026-06-13** in both.)
...
- **Blog soft-404 (deferred by user 2026-05-31)** — `/blog/<any-missing-slug>` returns **HTTP 200** instead of 404 (e.g. `/blog/this-slug-does-not-exist-xyz123` and the removed `/blog/volunteer-spotlight-maria`). `src/app/blog/[slug]/page.tsx` already calls `notFound()` (~line 51) + serves a `noindex` "Post Not Found" page, so Google won't index these (rankings safe) — but the status is wrongly 200 (response headers: `X-Nextjs-Prerender: 1`, `X-Vercel-Cache: HIT`, i.e. the notFound() branch is prerendered/CDN-cached as 200). Fix: make missing posts return a true 404 (investigate `generateStaticParams`/`dynamicParams`/prerender caching in Next 16; maybe add a `blog/[slug]/not-found.tsx`). Verify: the two curls return 404 while `/blog/record-breaking-2025` stays 200. GSC index report on 2026-05-31 was healthy (51 indexed / 13 not — the 13 are benign: canonical-alternates ×3, crawled-not-yet-indexed ×5, genuine old-URL 404s ×5; sitemap is 100% clean).
